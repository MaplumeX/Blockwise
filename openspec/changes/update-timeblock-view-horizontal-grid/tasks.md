...
- [ ] 1.3 撰写 spec delta：timeblock-view
...
- [ ] 2.1 将 TimeBlockDayView 的网格绘制与段渲染逻辑校准到“小时为行、分钟为列”的横向网格（24×12）
- [ ] 2.2 确认不支持横向滚动（输入手势/布局约束）
- [ ] 2.3 校验跨小时切片与圆角策略符合 spec
- [ ] 2.4 回归验证：Timeline 入口切换/偏好恢复/日期一致性
- [ ] 2.5 运行验证：`./gradlew test`、`./gradlew assembleDebug`
