...
- [ ] Timeline 内支持“列表 ↔ 时间块”同屏切换
- [ ] 视图偏好可持久化并在下次进入恢复
- [ ] 时间块视图具备 24×12（5 分钟）网格与段胶囊渲染
- [ ] 两视图共享同一选中日期与同一数据源，变更实时同步
- [ ] `./gradlew test` 与 `./gradlew assembleDebug` 通过
